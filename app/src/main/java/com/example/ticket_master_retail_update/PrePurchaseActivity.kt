...
import com.example.ticket_master_retail_update.PurchaseActivity
import com.example.ticket_master_retail_update.R
import com.example.ticket_master_retail_update.utils.hideKeyboard
...
//        }
        ...
        launchButton.setOnClickListener {
            ...
            try {
                ...
                if (!isFormComplete()) {
                    Log.w("PrePurchase", "Form is incomplete")
                    return@setOnClickListener
                }
                ...
                Log.d("PrePurchase", "Inputs - apiKey: $apiKey, region: $region, market: $market, clientName: $clientName, artistOrVenueId: $artistOrVenueId")

                val abstractEntity: DiscoveryAbstractEntity = if (entityType is DiscoveryEntity.Arist) {
                    Log.d("PrePurchase", "Entity type is Artist")
                    DiscoveryAttraction(hostID = artistOrVenueId)
                } else {
                    Log.d("PrePurchase", "Entity type is Venue")
                    DiscoveryVenue(hostID = artistOrVenueId)
                }

                // Validate the market enum
                val hostType = try {
                    TMMarketDomain.valueOf(market)
                } catch (e: IllegalArgumentException) {
                    Log.e("PrePurchase", "Invalid market value: $market", e)
                    throw IllegalArgumentException("Invalid market value: $market")
                }
                ...
                // Log before creating the fragment factory
                Log.d("PrePurchase", "Setting up TMPrePurchaseFragmentFactory")
                ...
                val factory = TMPrePurchaseFragmentFactory(
                    tmPrePurchaseNavigationListener = PrePurchaseNavigationListener(
                        ...
                        if (::fragment.isInitialized) {
                            Log.d("PrePurchase", "Removing fragment")
                            supportFragmentManager.beginTransaction().remove(fragment).commit()
                        } else {
                            Log.d("PrePurchase", "Finishing activity")
                            finish()
                        }
                    },
                    tmPrePurchaseFavoritesListener = PrePurchaseFavoriteListener(),
                ...
                // Log before committing the fragment
                Log.d("PrePurchase", "Adding fragment to supportFragmentManager")
                supportFragmentManager.beginTransaction()
                    .add(android.R.id.content, fragment)
                    .commit()
            } catch (e: Exception) {
                // Log the exception for debugging
                Log.e("PrePurchase", "Error occurred during launch", e)
